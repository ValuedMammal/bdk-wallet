...
fn make_generic_signature<M: Fn() -> SatisfiableItem, F: Fn(&Psbt) -> bool>(
    ...
    // Note: signers are no longer supported, so there's nothing to contribute
...
mod test {
    ...
    fn test_extract_policy_for_sh_multi_partial_1of2() {
        ...
        dbg!(&policy.contribution);

        assert_matches!(&policy.contribution, Satisfaction::Partial { .. });
        // assert_matches!(&policy.contribution, Satisfaction::Partial { n, m, items, conditions,
        // ..} if n == &2usize      && m == &2usize
        //      && items.len() == 1
    ...
    fn test_extract_policy_for_sh_multi_complete_2of2() {
        ...
        assert_matches!(&policy.contribution, Satisfaction::Partial { .. });
        // assert_matches!(&policy.contribution, Satisfaction::PartialComplete { n, m, items,
        // conditions, .. } if n == &2      && m == &2
        //      && items.len() == 2
    ...
    fn test_extract_policy_for_single_wpkh() {
        ...
        assert_matches!(policy.item, EcdsaSignature(PkOrF::Fingerprint(f)) if f == fingerprint);
        // assert_matches!(policy.contribution, Satisfaction::Complete {condition} if
        // condition.csv.is_none() && condition.timelock.is_none());
    }
    ...
    fn test_extract_tr_key_spend() {
        ...
        let (prvkey, pubkey, fingerprint) = setup_keys(ALICE_TPRV_STR, ALICE_BOB_PATH, &secp);
