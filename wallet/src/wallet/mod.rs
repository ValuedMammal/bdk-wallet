...
impl Wallet {
    ...
    pub fn load_with_params(
        ...
        wallet.utxo_info = wallet
            ...
            .map(|output| {
                ...
                (output.outpoint, utxo_info)
            })
            .collect();
        for (outpoint, is_locked) in changeset.utxo_info {
            if is_locked {
                wallet.lock_unspent(outpoint);
            }
        }
    ...
    /// Is utxo locked.
    pub fn is_utxo_locked(&self, op: OutPoint) -> Option<bool> {
        Some(self.utxo_info.get(&op)?.is_locked)
    }
    ...
    pub fn lock_unspent(&mut self, outpoint: OutPoint) {
        ...
        if is_changed {
            let changeset = ChangeSet {
                utxo_info: [(outpoint, new_lock)].into(),
                ..Default::default()
            };
            self.stage.merge(changeset);
        }
