...
impl GetKey for SignerWrapper {
    ...
    fn get_key<C: bitcoin::secp256k1::Signing>(
        ...
        // iterations.
        // review: You could collect the errors in a vec, and if we've iterated through the keymap
        // and didn't find a result then return the first error encountered, or else None.
        let mut errors = vec![];

        for item in self.key_map.iter() {
            let (_, desc_sk) = item;
            // review: This should call `.into_single_keys()` on the DescriptorSecretKey and have 1
            // more inner loop. This removes the last case of MultiXprv by consolidating
            // the logic into the Xprv branch.
...
impl GetKey for DescriptorSecretKeyWrapper {
    ...
    fn get_key<C: bitcoin::secp256k1::Signing>(
        ...
        match (&self.0, key_request) {
            ...
            (DescriptorSecretKey::MultiXPrv(_), _) => {
                unreachable!("Bdk should never call this with a MultiXPrv")
            }
...
// This appears frivolous from an API perspective.
